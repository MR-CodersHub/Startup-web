...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Use default user object since no authentication required
    const user = {
        id: 1,
        name: 'Guest User',
        email: 'guest@startupweb.com',
        role: 'user',
    ...
    function renderLogs() {
        ...
        logsTableBody.innerHTML = logs.map(log => `
            <tr class="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                <td data-label="Action" class="font-medium text-gray-800">${log.action}</td>
                <td data-label="Details" class="text-gray-600 text-sm">${log.details}</td>
                <td data-label="Time" class="text-gray-400 text-xs">${new Date(log.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</td>
            </tr>
        `).join('');
